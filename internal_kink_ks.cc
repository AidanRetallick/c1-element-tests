...
#include <iostream>
#include "c1_koiter_steigmann/koiter_steigmann_curvable_bell_elements.h"
...
int main()
{
  ...
  KoiterSteigmannC1CurvableBellElement right_el_straight;
  KoiterSteigmannC1CurvableBellElement left_el_straight;
  KoiterSteigmannC1CurvableBellElement right_el_cubic;
  KoiterSteigmannC1CurvableBellElement left_el_cubic;
  KoiterSteigmannC1CurvableBellElement right_el_quintic;
  KoiterSteigmannC1CurvableBellElement left_el_quintic;
  // Array of element pointers
  // (named for what it is Matthias, not what it contains)
  KoiterSteigmannC1CurvableBellElement* right_el_pt_array[3] =
    {&right_el_straight, &right_el_cubic, &right_el_quintic};
  KoiterSteigmannC1CurvableBellElement* left_el_pt_array[3] =
    {&left_el_straight, &left_el_cubic, &left_el_quintic};
  for(unsigned i = 0; i < 3; i++)
  {
    KoiterSteigmannC1CurvableBellElement* right_el_pt = right_el_pt_array[i];
    KoiterSteigmannC1CurvableBellElement* left_el_pt = left_el_pt_array[i];
    Node* n0 = right_el_pt->construct_node(0);
    Node* n1 = right_el_pt->construct_node(1);
    Node* n2 = right_el_pt->construct_node(2);
    Node* n3 = left_el_pt->construct_node(2);
    left_el_pt->node_pt(0) = n1;
    left_el_pt->node_pt(1) = n0;
    n0->x(0) = v0[0];
    n0->x(1) = v0[1];
    n1->x(0) = v1[0];
    n1->x(1) = v1[1];
  ...
  right_el_cubic.upgrade_element_to_curved(edge_number, 0.0, 1.0, curviline_straight_edge_pt, 3);
  left_el_cubic.upgrade_element_to_curved(edge_number, 1.0, 0.0, curviline_straight_edge_pt, 3);
  right_el_quintic.upgrade_element_to_curved(edge_number, 0.0, 1.0, curviline_straight_edge_pt, 5);
  left_el_quintic.upgrade_element_to_curved(edge_number, 1.0, 0.0, curviline_straight_edge_pt, 5);
  ...
  {
      KoiterSteigmannC1CurvableBellElement* right_el_pt = right_el_pt_array[i_el];
      KoiterSteigmannC1CurvableBellElement* left_el_pt = left_el_pt_array[i_el];
      unsigned n_internal_type = right_el_pt->ninternal_basis_type_for_field(2);
      ...
      for (unsigned i = 0; i < n_node; i++)
      {
          for (unsigned j = 0; j < n_nodal_type; j++)
          {
              ...
              right_el_pt->node_pt(i)->set_value(w_index + j, 1.0);
              right_el_pt->output_full(outfile, 100);
              left_el_pt->output_full(outfile, 100);
              right_el_pt->node_pt(i)->set_value(w_index + j, 0.0);
              outfile.close();
          }
      }
      for (unsigned j = 0; j < n_internal_type; j++)
      {
          ...
          std::ofstream outfile(filename);
          right_el_pt->internal_data_pt(2)->set_value(j, 1.0);
          right_el_pt->output_full(outfile, 100);
          left_el_pt->output_full(outfile, 100);
          right_el_pt->internal_data_pt(2)->set_value(j, 0.0);
          outfile.close();
      }
  }
